...
class GroundStation {
    constructor() {
        ...
        this.RETRANSMISSION_TIMEOUT = 10000; // Increased to 20 seconds
    ...
    async launchServer() {
        try {
            ...
            while (
                ...
                while (true) {
                    ...
                    if (chunk) {
                        ...
                        console.log(
                            new Date().toISOString() + " Timeout refreshed"
                        );
                    }
                ...
                if (
                    ...
                    const headerData = chunkBytes.slice(
                        0,
                        this.PROTOCOL_HEADER_SIZE
                    );
                    const preamble = new TextDecoder().decode(
                        headerData.slice(0, 4)
                    );
                    ...
                    this.log(
                        `Detected ${this.width}x${this.height} image`,
                        "info"
                    );
                ...
                if (chunkBytes.length > 2) {
                    const seqNumber = new DataView(chunkBytes.buffer).getUint16(
                        0,
                        false
                    );
                    ...
                    if (this.chunksReceived[seqNumber]) {
                        this.log(
                            `Received duplicate chunk ${seqNumber}, ignoring`,
                            "info"
                        );
                        continue;
                    }
            ...
            this.log(
                `Saved ${this.incomingBytes} bytes to "bytes.bin"`,
                "success"
            );
    ...
    updateDronePosition() {
        ...
        if (typeof updateDronePosition === "function") {
            ...
            this.log(
                `Updated drone position: ${randomLat.toFixed(
                    6
                )}, ${randomLng.toFixed(6)}`,
                "info"
            );
            ...
            if (this.reader) {
                ...
                if (this.readableStreamClosed) {
                    try {
                        ...
                        this.log(
                            `Read stream closed with error: ${e}`,
                            "error"
                        );
                    }
                }
            ...
            if (this.writer) {
                ...
                if (this.writableStreamClosed) {
                    try {
                        ...
                        this.log(
                            `Write stream closed with error: ${e}`,
                            "error"
                        );
                    }
                }
  ...
  AT+TEST=RFCFG,${this.RF_CONFIG.frequency},SF${
        ...
        },${this.RF_CONFIG.bandwidth},12,15,${
            this.RF_CONFIG.powerDbm
        },ON,OFF,OFF`;
    }
}
